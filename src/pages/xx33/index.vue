    ...
    <div class="bg-color">
      ...
      <div class="section flex-h" v-if="getGx!='∞'">
        <view class="weui-cells__title section-title half-width">故障处理完毕<span class="red">{{!isGx ? '正向有功' : '反向有功'}}</span>总电量示值(kWh)</view>
          <view class="weui-cell weui-cell_input  line-2-v-center">
            <input class="weui-input  felix-input felix-right-input" v-model="w2" type="digit"   @blur="fnBlurInputW2()" @input="fnInputW2()" maxlength="9" placeholder="请输入{{!isGx ? '正向' : '反向'}}有功总电量"/>
          </view>
      </div>
      <div class="section flex-h" v-if="getGx!='∞'">
...
export default {
  ...
  methods: {
    ...
    error_conn_calc(){
      ...
      this.isGx = parseFloat(gx) <= 0;
      this.preFix = this.model.getPreFix();
      let rate =this.objectdyhgbs[this.dyhgbIndex].rate * this.dlhgb;
      this.model.k = rate;
      ...
      const tdl = this.isGx ? (1 - this.isGx) * this.w : (0 - this.isGx) * this.w;

      const p = Math.abs(this.dj * tdl);

      this.dialog.hiddenDialog = false;
      this.dialog.p = this.model.getP();
      this.dialog.rate = this.model.k;
      this.dialog.gx = this.model.gx;
      this.dialog.wz = parseFloat(this.w2 - this.w1) * parseFloat(rate);
      console.log("-----"+ parseFloat(this.w2 - this.w1) +"::::"+ parseFloat(rate));
  ...
  computed:{
    ...
    getGx(){
      ...
      gx = gx == null ? '∞' : parseFloat(gx).toFixed(4);
      this.isGx = parseFloat(gx) <= 0;
      return gx;
    }
