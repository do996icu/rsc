<template>
  <div class="bg-color">
    ...
    <felix-radio v-bind:hidden="faultIndex != 0" title="表尾电流接线：" v-bind:is-mark="true" :items="iConnections"  @getResult="onFRIchange"></felix-radio>
    ...
    <view v-if="getGx != '∞'" class="weui-cells__title section-mode-title">电能表<span class="red">{{!isGx ? '正向有功' : '反向有功'}}</span>抄见电量：</view>
...
<script>
  import gxParam from '../../utils/gx34Util';
  ...
  let iConnections = [{name: 'IaIbIc', checked: true},
    {name: 'IbIcIa', checked: false},
    {name: 'IcIaIb', checked: false},
    {name: 'IaIcIb', checked: false},
    {name: 'IbIaIc', checked: false},
    {name: 'IcIbIa', checked: false}
    ];
  let elements = [{
    name: 'first',
    checked: false,
  ...
  const dyhgbs = ['0.38kV/0.38kV', '10kV/0.1kV', '35kV/0.1kV', '66kV/0.1kV', '110kV/0.1kV', '220kV/0.1kV', '330kV/0.1kV'];
  const objectdyhgbs = [{name:'0.38kV/0.38kV', rate: 1},{name:'10kV/0.1kV',  rate: 100 }, {name:'35kV/0.1kV',rate: 350}, {name:'66kV/0.1kV', rate: 660}, {name:'110kV/0.1kV', rate: 1100}, {name:'220kV/0.1kV', rate: 2200},{name:'330kV/0.1kV', rate: 3300}];
  const mulitLeftArr = [];
  ...
  export default {
    ...
    methods: {
      ...
      error_conn_calc(){
        ...
        let gxId = this.faultIndex == 0 ? [this.vc, this.vi, 'first', !this.elements[0].checked, 'second', !this.elements[1].checked, 'third', !this.elements[2].checked].join('_') : this.vc;
        let gxFns = this.gxParam[this.faultIndex].filter((item)=>item[gxId]);
        ...
        this.dialog.rate = rate;
        this.dialog.gx = this.model.gx;
        this.dialog.wz = this.model.dW * rate;
